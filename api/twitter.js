...
export default async function handler(req, res) {
  try {
    ...
    const query = 'AI news -is:retweet lang:en';
    ...
    if (status === 429) {
      return res.status(200).json({
        items: [],
        source: 'twitter',
        message: 'Twitter API rate limit reached. Free tier allows ~1500 tweets/month. Try again in 15 minutes.',
        rateLimited: true
      });
    }

    console.error('Twitter API error:', status, errorData || error.message);
    res.status(200).json({
